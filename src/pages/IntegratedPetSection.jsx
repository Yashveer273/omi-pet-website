...
export default function IntegratedPetSection() {
  ...
  const [metaConfig, setMetaConfig] = useState({
    combinedBreedFilters: DEFAULT_BREED_FILTERS,
  });
    ...
    const loadMetaConfig = async () => {
      try {
        const metaDoc = await getDoc(
          doc(firestore, "metadata", "configuration"),
        );
        if (metaDoc.exists()) {
          const metaData = metaDoc.data();
          setMetaConfig({
            ...metaData,
            combinedBreedFilters: mergeBreedFilters(
              metaData.combinedBreedFilters,
            ),
          });
        }
      } catch (error) {
  ...
  const PET_DATA_STORE = {
    ...
    subheading:
      "Explore Dogs, Cats, Birds, Fish, Small Pets, and Trusted Veterinary Care Services",
    speciesTabs: [
      {
        id: "dog",
        name: "Dog",
  ...
  const activeSpeciesConfig =
    PET_DATA_STORE.speciesTabs.find((t) => t.id === activeTab) ||
    PET_DATA_STORE.speciesTabs[0];
  const activeLabelPrefix = activeSpeciesConfig.prefix;

  const computedServices = [
    {
      id: "breed",
      title: `${activeLabelPrefix} Breeds`,
      desc: "Discover healthy breeds with loving personalities and complete family care metrics.",
      image:
    ...
    {
      id: "breeder",
      title: `${activeLabelPrefix} Breeders`,
      desc: "Trusted, verified professionals focusing on health, vaccination records, and genetics.",
      image:
    ...
    {
      id: "trainer",
      title: `${activeLabelPrefix} Trainers`,
      desc: "Professional coaches specializing in smart obedience and behavioral training.",
      image:
    ...
    {
      id: "walker",
      title: `${activeLabelPrefix} Walkers`,
      desc: "Reliable handlers to keep your companions fit, energetic, and happy.",
      image:
    ...
    {
      id: "food",
      title: `${activeLabelPrefix} Food`,
      desc: "Premium nutritional diets tailored for optimal development and health.",
      image:
    ...
    {
      id: "accessories",
      title: `${activeLabelPrefix} Accessories`,
      desc: "Shop elite accessories, ergonomic harnesses, and luxury pet essentials.",
      image:
    ...
    {
      id: "stay",
      title: `${activeLabelPrefix} Stay Centres`,
      desc: "Premium climate-controlled luxury boarding with 24/7 monitoring.",
      image:
    ...
    {
      id: "medicine",
      title: `${activeLabelPrefix} Medicines`,
      desc: "Prescription support, preventive vitamins, and targeted supplements.",
      image:
  ...
  const getBreedLabel = (id) =>
    PET_DATA_STORE.combinedBreedFilters.find((item) => item.id === id)?.label ||
    id;
  const getBreedStageLabel = (id) =>
    BREED_STAGE_OPTIONS.find((item) => item.id === id)?.label || id;
  const filteredBreeds = currentRegistryList.filter((b) => {
    const query = searchQuery.trim().toLowerCase();
    const matchesSearch =
      !query ||
      (b.name || "").toLowerCase().includes(query) ||
      (b.breedId || "").toLowerCase().includes(query);
    const matchesFilter =
      selectedFilter === "all" ||
  ...
  return (
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12">
          <h2 className="text-3xl sm:text-4xl font-black text-stone-900 tracking-tight mb-3">
            {PET_DATA_STORE.heading}
          </h2>
          <p className="text-sm text-stone-500 font-medium leading-relaxed">
            {PET_DATA_STORE.subheading}
          </p>
        </div>
        ...
        <div className="w-full relative overflow-hidden mb-14 py-2">
          <div className="animate-infinite-marquee gap-3 flex items-center">
            {[...PET_DATA_STORE.speciesTabs, ...PET_DATA_STORE.speciesTabs].map(
              (tab, index) => {
    ...
    w-28 h-32 shrink-0 mx-2 rounded-2xl border transition-all duration-300
    ${
      isSelected
        ? "bg-stone-900 border-stone-900 shadow-lg scale-105"
              ...
              <SwiperSlide key={service.id}>
                {({ isActive }) => (
                  <div
                    ...
                    className={`bg-white rounded-3xl overflow-hidden cursor-pointer transition-all duration-500 border
    ${
      isActive
        ? "scale-100 shadow-xl border-stone-300"
        : "scale-95 opacity-70 border-stone-100"
    }`}
                  >
                    {/* Image Section */}
                    ...
                    <div className="p-5">
                      ...
                      <div className="mt-5 flex items-center justify-between text-stone-900 font-bold text-xs uppercase tracking-wider">
                        <span>Explore</span>
          ...
          <div className="flex justify-center gap-3 mt-4">
            <button className="service-prev w-10 h-10 rounded-full bg-white border border-stone-200 shadow-xs flex items-center justify-center">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <button className="service-next w-10 h-10 rounded-full bg-white border border-stone-200 shadow-xs flex items-center justify-center">
              <ChevronRight className="w-4 h-4" />
            </button>
          </div>
        </div>
        ...
        <div className="hidden lg:grid grid-cols-3 xl:grid-cols-4 gap-6">
          {computedServices.map((service) => (
            <div
              key={service.id}
              onClick={() => handleCategoryClick(service)}
              className="group bg-white rounded-2xl overflow-hidden border border-stone-200/80 cursor-pointer transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl hover:border-stone-400"
            >
              {/* Image Section */}
              ...
              <div className="p-6 flex flex-col min-h-[180px]">
                ...
                <div className="mt-6 pt-4 border-t border-stone-100 flex items-center justify-between text-stone-900 font-bold text-xs uppercase tracking-wider">
                  ...
                  <div className="w-8 h-8 rounded-full bg-stone-50 border border-stone-200 flex items-center justify-center transition-transform group-hover:translate-x-1">
                    <ChevronRight className="w-4 h-4" />
                  </div>
                </div>
              </div>
          ...
          <div className="w-full h-full flex flex-col bg-white">
            ...
            <div className="px-6 py-5 sm:px-10 bg-white border-b border-stone-200 flex items-center justify-between sticky top-0 z-20">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 text-2xl rounded-2xl bg-stone-50 border border-stone-200 flex items-center justify-center shadow-3xs">
                  ...
                  <div className="space-y-6 pt-2">
                    {filteredBreeds.length > 0 ? (
                      filteredBreeds.map((breed) => (
                        <div
                          key={breed.breedId}
                          className="bg-[#FCFCFD] border border-stone-200 rounded-2xl p-6 flex flex-col lg:flex-row gap-8 hover:border-stone-300 transition-all shadow-3xs"
                        >
                          {/* Left Panel: Stats */}
                          <div className="flex-1 space-y-4 text-left lg:max-w-xs lg:border-r lg:border-stone-200 lg:pr-8 pb-4 lg:pb-0">
                            <div>
                              <h6 className="text-2xl font-black text-stone-900 tracking-tight">
                                {breed.name}
                              </h6>
                            ...
                            <div className="space-y-2.5 text-xs text-stone-600 bg-white p-4 rounded-xl border border-stone-200 shadow-3xs">
                              <div className="flex items-center justify-between border-b border-stone-100 pb-2">
                                ...
                                <strong className="text-stone-800 font-semibold">
                                  {getBreedStageLabel(breed.breedStage)}
                                </strong>
                              </div>
                              <div className="flex items-center justify-between border-b border-stone-100 pb-2">
                                <span className="text-stone-400">Breed</span>
                                <strong className="text-stone-800 font-semibold">
                                  {getBreedLabel(breed.traitGroup)}
                                </strong>
                              </div>
                              <div className="flex items-center justify-between border-b border-stone-100 pb-2">
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <strong className="text-stone-800 font-semibold capitalize">
                                  {breed.sizeGroup?.replace("-", " ")}
                                </strong>
                              </div>
                            </div>
                          ...
                          <div className="flex-2">
                            <div className="border border-stone-200 rounded-2xl overflow-hidden bg-white flex flex-col justify-between group shadow-3xs h-full">
                              <div className="h-48 bg-stone-100 overflow-hidden relative">
                                <img
                                  src={breed.puppyImg || breed.adultImg}
                                  alt={breed.name}
                                  className="w-full h-full object-cover"
                                />
                      ...
                      <div className="py-20 text-center border border-dashed border-stone-200 rounded-2xl bg-stone-50">
                        <p className="text-xs text-stone-400 font-bold uppercase tracking-widest">
                          No configurations discovered matching search.
                        </p>
                      </div>
                    )}
                  </div>
                      ...
                      {selectedService.uiType === "expertGrid" && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
                          {directoryList.map((expert) => (
                            <div
                              key={expert.id}
                              className="bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-3xs hover:border-stone-300 transition-all"
                            >
                              <img
                                src={expert.img}
                                alt={expert.title}
